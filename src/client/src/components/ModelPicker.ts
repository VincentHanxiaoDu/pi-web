...
export interface ModelCatalogView {
  ...
  /** Empty query only: rows mix enabled and disabled entries, so the picker
   * draws its group headings. A non-empty query flattens the list. */
...
export function modelCatalogView(
  ...
  const rows = normalized === ""
    ? orderedRows
    : orderedRows.filter((entry) => matchesAllQueryWords(`${entry.provider} ${entry.id} ${entry.name ?? ""}`, normalized));
...
export class ModelPicker extends LitElement {
  ...
  static override styles = css`
    ...
    @media (hover: hover) { .options > button:hover { background: var(--pi-selection-bg); } .toggle-all:hover:not(:disabled) { background: var(--pi-selection-bg); } }
